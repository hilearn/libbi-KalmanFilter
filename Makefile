...
ifeq ($(second), $(word 2, $(MAKECMDGOALS)))
	${_python} -m coverage erase
	${_python} -m nose -v tests/*/*.py --with-coverage --cover-no-print --logging-level=ERROR
	@echo "#####################"
	@echo "##       OK        ##"
	@echo "#####################"
	${_python} report_coverage.py
else
	${_python} -m nose $(word 2, $(MAKECMDGOALS)) --logging-level=ERROR --nocapture
endif
.PHONY: test
